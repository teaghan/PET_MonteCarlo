...
def create_phantom(phantom_size, dz, dy, dx, ph_type, y_off=0, x_off=0, rot=0):
    '''
    Create digital form of uniform phantom.
    ...
    if ph_type=='attn':
        ...
        x0 = np.round(plane.shape[0]/2 + x_off)
        y0 = np.round(plane.shape[1]/2 + y_off)
        # Find pixels inside circle
        inside = (d/2)**2 - (x0-xlocs)**2 - (y0-ylocs)**2
        # Set housing pixels to 2
        plane[inside>=0] = 2
    
    # Create interior solution of activity
    ...
    print('The volume of the cylinder is %0.2f L' % (len(np.where(phantom==1)[0])*dx*dy*dz*1e-6))

    if rot!=0:
        from scipy.ndimage import rotate
        phantom = rotate(phantom, rot, axes=(1, 0), reshape=False)
        phantom = np.rint(phantom)
    return phantom
...
def save_phantom(phantom, data_dir, ph_type):
    ...
    inputs:
        phantom (np.array): 3D array containing cylindrical phantom.
        data_dir (str): path to data directory where the array will be saved.
    '''
    phantom_fn = os.path.join(data_dir, 'uniform_%s.bin'%ph_type)
...
def save_atten_map(data_dir, recon_dir):
    ...
    attn_ph = np.fromfile(os.path.join(data_dir, 'uniform_attn.bin'), dtype=np.int16).astype(np.float32)

    # Obtain image size and resolution
    hdr_fn = os.path.join(data_dir,'uniform_attn.h33')
...
def save_header(data_dir, phantom_size, dz, dy, dx, ph_type):
    '''Write header file.'''
    hdr_fn = os.path.join(data_dir, 'uniform_%s.h33'%ph_type)
...
def save_ranges(data_dir, dz, dy, dx, cyl_act, ph_type):
    '''
    ...
    if ph_type=='attn':
        ...
        with open(atn_rng_fn,'w') as file:
            ...
            file.write('0 0 Air\n')
            # Water
            file.write('1 1 Water\n')
            # Polyethylene housing
            file.write('2 2 Polyethylene\n')
